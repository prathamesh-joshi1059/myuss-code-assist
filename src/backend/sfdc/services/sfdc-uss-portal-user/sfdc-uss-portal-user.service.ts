...
export class SfdcUssPortalUserService {
  private readonly MYUSS_DEFAULT_MODULES: [];
  ...
  async getUserContext(auth0UserId: string): Promise<USS_Portal_User__c> {
    ...
    const resp = await this.sfdcBaseService.getQuery(query);
    // TODO: handle more than one record, which should never happen
    ...
    //TODO: [SKW-4] create an endpoint to invalidate user context cache in case something changes in SFDC
    await this.cacheService.set(cacheKey, ussPortalUser, 300);
    // TODO: [SKW-3] convert to DTO
    return ussPortalUser;
  }
  ...
  async createUser(ussPortalNewUser: SignupReqDTO): Promise<SfdcRespModel> {
    try {
      ...
      //   return { message: 'User already exists', success: true , errors: [] , id: ''};
      // }
      //step 1 create object for USS_Portal_User__c
      ...
      if (existingUser.length == 0)
        respOfUssPortalUser = await this.sfdcBaseService.createSObject('USS_Portal_User__c', ussPortalUserObj);
      else respOfUssPortalUser = { success: true, id: existingUser[0].Id };
      if (respOfUssPortalUser.success) {
        //step 2 create object for Account
        ...
        if (respOfAccount.success) {
          //step 3 create object for Contact
          ...
          if (respOfContact.success) {
            ...
            // if not found, create the ACR
            if (acrResp.records.length == 0) {
              const respOfAccountContactRelation = await this.sfdcBaseService.createSObject('AccountContactRelation', {
              ...
              const respOfAccountContactRelation = await this.sfdcBaseService.updateSObject(
            ...
            if (respOfContact.success) {
              ...
              {
                this.logger.error('err in AccountContactRelation');
                return { message: 'err in AccountContactRelation', success: false, errors: [], id: '' };
              }
            }
          } else {
  ...
  public async upsertPortalUsers(users: USS_Portal_User__c[]): Promise<USS_Portal_User__c[]> {
    ...
    // get the updated records with email addresses
  ...
  async fetchUserContactListForAccount(fetchUsersFromAccountReqDto:FetchUserContactsForAccountReqDto){
    let query = '';
    if(fetchUsersFromAccountReqDto.isAllContacts){
       query = `select id, ContactId, Contact.Phone,Contact.Name, Account.Name, AccountId, MyUSS_User_Role__c, MyUSS_Modules__c, CreatedBy.Name, IsActive, Roles,Contact.USS_Portal_User__r.Id,Contact.USS_Portal_User__r.Email_Address__c, Contact.USS_Portal_User__r.Auth0_Id__c from AccountContactRelation limit 50`;
    }else{
       query = `select id, ContactId,Contact.Phone, Contact.Name, Account.Name, AccountId, MyUSS_User_Role__c, MyUSS_Modules__c, CreatedBy.Name, IsActive, Roles,Contact.USS_Portal_User__r.Id,Contact.USS_Portal_User__r.Email_Address__c, Contact.USS_Portal_User__r.Auth0_Id__c from AccountContactRelation where Contact.USS_Portal_User__c != '' AND AccountId = '${fetchUsersFromAccountReqDto.accountId}'`;
   }
  ...
  async editUserContactForAccount(editUserContactReqDto:AddUpdateUserContactForAccountReqDto){
    ...
    let contactObj = new ContactDto();
      ...
      if (createContactResultRespModel.success) {
         ...
         // accountContactRelation.IsActive = editUserContactReqDto.isActive;
          ...
          const accountContactRelationRespModel = new SfdcRespModel();
            ...
            if (accountContactRelationRespModel.success) {
                  return true;
            } else {
                  return false;
            }
